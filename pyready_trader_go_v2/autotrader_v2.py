...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if self.event_loop.time() - self.start_time >= 50:
            if self.position - self.hedge_position > 0:
                self.send_hedge_order(next(self.order_ids), Side.BID, MAX_ASK_NEAREST_TICK, self.position - self.hedge_position)
                self.hedge_position = self.position
            elif self.hedge_position - self.position > 0:
                self.send_hedge_order(next(self.order_ids), Side.ASK, MIN_BID_NEAREST_TICK, self.hedge_position - self.position)
                self.hedge_position = self.position
        ...
        if instrument == Instrument.FUTURE:
            ...
            new_bid_volume = math.ceil((POSITION_LIMIT - self.position) / 2) if self.position < 0 else math.floor((POSITION_LIMIT - self.position) / 2)
            new_ask_volume = self.position - new_bid_volume

            self.cancel_bid_order(new_bid_price)
            self.insert_bid_order(new_bid_price, new_bid_volume, Lifespan.GOOD_FOR_DAY)
            
            self.cancel_ask_order(new_ask_price)
            self.insert_ask_order(new_ask_price, new_ask_volume, Lifespan.GOOD_FOR_DAY)
